...
{
    private record DealTableRecord(int Id, string Company, string Stage, string Amount, string CloseDate);
    ...
    public override object? Build()
    {
        ...
        var (sheetView, showSheet) = UseTrigger((IState<bool> isOpen, int id) => new ContactDealsEditSheet(isOpen, refreshToken, id));
        ...
        var dataTableKey = $"deals-{contactId}-{tableData.Length}-{tableData.Aggregate(0, (h, d) => HashCode.Combine(h, d.Id))}";
