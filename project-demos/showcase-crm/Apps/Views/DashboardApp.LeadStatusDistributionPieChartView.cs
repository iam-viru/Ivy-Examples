/*
Analyze the current status of leads to understand conversion funnel performance
GROUP leads BY Status.DescriptionText WHERE CreatedAt BETWEEN startDate AND endDate, COUNT(*) AS status_count
*/
namespace ShowcaseCrm.Apps.Views;

public class LeadStatusDistributionPieChartView(DateTime startDate, DateTime endDate) : ViewBase
{
    private record ChartData(string Status, double Count);

    public override object? Build()
    {
        var query = UseChartData(Context);
        var card = new Card().Title("Lead Status Distribution").Height(Size.Units(80));
    ...
    private QueryResult<ChartData[]> UseChartData(IViewContext context)
    {
        ...
        return context.UseQuery(
            key: (nameof(LeadStatusDistributionPieChartView), startDate, endDate),
            fetcher: async ct =>
            {
                await using var db = factory.CreateDbContext();
                var data = await db.Leads
                    .Where(l => l.CreatedAt >= startDate && l.CreatedAt <= endDate)
                    .GroupBy(l => l.Status.DescriptionText)
